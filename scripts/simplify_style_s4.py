"""
SilentCare - Simplify Style (Section 4)
=========================================
Applies exact text replacements to simplify vocabulary in Section 4.
Does NOT modify tables, metrics, figure captions, or section headers.

Usage:
    python scripts/simplify_style_s4.py
"""
...
REPLACEMENTS = [
    (
        "leveraging two deep learning frameworks to exploit the best available "
        "pre-trained models for each modality",
        "using two deep learning frameworks to get the best pre-trained models "
        "for each modality",
    ),
    (
        "Weighted random sampling was used during training to mitigate this imbalance.",
        "Weighted random sampling was used during training to handle this imbalance.",
    ),
    (
        "No existing dataset adequately represents non-verbal emotional sounds "
        "for monitoring scenarios. A custom dataset of 1,876 audio files was "
        "therefore assembled from multiple sources",
        "No existing dataset covered non-verbal emotional sounds well enough "
        "for this use case. I built a custom dataset of 1,876 audio files from "
        "multiple sources",
    ),
    (
        "as required for realistic cross-domain generalization evaluation.",
        "to properly test how well each model generalizes.",
    ),
    (
        "making inference time a critical design constraint.",
        "which made inference speed a key constraint.",
    ),
]
...
def main():
    print("=" * 60)
    print("SilentCare - Simplify Style (Section 4)")
    print("=" * 60)
